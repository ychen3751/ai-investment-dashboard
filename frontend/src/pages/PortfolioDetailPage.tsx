...
import { fetchPortfolio, fetchHoldings, addHolding, removeHolding, deletePortfolio, fetchPerformance } from '../api/portfolios'
...
export function PortfolioDetailPage() {
  ...
  const existingSymbols = new Set((holdings || []).map((h) => h.symbol.toUpperCase()))
  ...
  const addHoldingMut = useMutation({
    ...
    onError: (error: AxiosError<{ detail?: string }>) => {
      ...
      setErrors({ general: typeof detail === 'string' ? detail : error.message || 'Failed to add holding' })
    },
  })
  ...
  const validate = useCallback((): boolean => {
    ...
    if (sym && existingSymbols.has(sym)) e.symbol = 'This symbol already exists in this portfolio'
    setErrors(e)
    return Object.keys(e).length === 0
  }, [symbol, quantity, avgCost, existingSymbols])
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <Button variant="danger" size="sm" onClick={async () => { await deletePortfolio(id!); navigate('/portfolios') }}>Delete</Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <MetricCard
          label="Total P&L"
          value={fmtCurrency(portfolio.total_pnl)}
          changePct={num(portfolio.total_pnl_pct)}
          change={portfolio.total_pnl != null ? fmtCurrency(portfolio.total_pnl) : undefined}
        />
      ...
      {perf && (
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <MetricCard label="Return" value={perf.total_return_pct != null ? fmtPct(perf.total_return_pct) : '-'} />
          <MetricCard label="Volatility" value={perf.volatility_pct != null ? fmtPct(perf.volatility_pct) : '-'} />
          <MetricCard label="Sharpe" value={perf.sharpe_ratio != null ? num(perf.sharpe_ratio).toFixed(2) : '-'} />
          <MetricCard label="Max Drawdown" value={perf.max_drawdown_pct != null ? fmtPct(perf.max_drawdown_pct) : '-'} />
        </div>
      )}
        ...
        {showForm && (
          <div className="flex flex-col gap-3 mb-4 p-4 bg-gray-800/50 rounded-lg border border-gray-700/50">
            ...
            <div className="flex gap-2">
              <Button onClick={handleSubmit} disabled={addHoldingMut.isPending}>
                ...
                {holdings.map((h) => (
                  <tr key={h.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                    ...
                    <td className="py-2 px-2 text-right">
                      <Button variant="ghost" size="sm" onClick={() => removeHoldingMut.mutate(h.id)} title="Remove holding">✕</Button>
                    </td>
                  </tr>
                ))}
              </tbody>
